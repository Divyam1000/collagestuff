...
node* createNode(int s, int k, int r, int *_x, int _level, int type)
{
...
void printTree()
{
    ...
    while(_stackTop != 0)
    {
        ...
        printf("Node %d ( %d %d %d ) ",_stackTop,stack[_stackTop]->s+1,stack[_stackTop]->k+1,stack[_stackTop]->r);
...
void sumOfSub(int s, int k, int r) {
    ...
    if (s + w[k] == m) 
    {
        pushOnStack(createNode(s+w[k],k,r-w[k],x,level,soloution_node));
        pushOnStack(createNode(s,k-1,r,x,level-1,non_leaf_node));
    }
    // no solution possible with this combination of x
    // else if(k == n-1)   
    // {  
    //     pushOnStack(createNode(s+w[k],k,r-w[k],x,level,leaf_node));   // leaf node without a solution
    //     pushOnStack(createNode(s,k-1,r,x,level-1,non_leaf_node));
    // }
    ...
    {
        // pushOnStack(createNode(s+w[k],k,r-w[k],x,level,leaf_node));
        pushOnStack(createNode(s,k-1,r,x,level-1,non_leaf_node));
    }
    ...
    {
        x[k] = 0;
        pushOnStack(createNode(s,k,r-w[k],x,level,leaf_node));   // leaf node without a solution
    }
...
int main() {
    ...
    pushOnStack(createNode(0,0,sum,x,0,non_leaf_node));    
    sumOfSub(0,0,sum); 
    printTree();
